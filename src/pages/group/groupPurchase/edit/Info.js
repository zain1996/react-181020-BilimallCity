...
export default class Info extends PureComponent {
  ...
  remove = (k) => {
    const { form ,dispatch,data,GroupState } = this.props;
    ...
    let Info = data.Info.filter((val,key) => key !== k);
  ...
  render(){
    ...
    const {data:{Info},GroupState} = this.props;
        ...
        >
          ...
          >
            {getFieldDecorator(`Info[${index}].Title`, {
              ...
              <Input disabled={GroupState > 1 ? true : false} placeholder={getPlaceholder(index)}/>
            )}
          ...
          >
            {getFieldDecorator(`Info[${index}].Content`, {
              ...
              <Input disabled={GroupState > 1 ? true : false} placeholder="请输入选项"/>
            )}
    ...
    return(
      <div>
        ...
        <FormItem style={{border:'1px solid #d9d9d9',padding:5,width:'100%',textAlign:'center'}}>
          <Button disabled={GroupState > 1 ? true : false} onClick={this.add}>
            增加一栏
          </Button>
        </FormItem>
      </div>
