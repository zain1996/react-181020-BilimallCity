...
export default class Combination extends PureComponent {
  ...
  render(){
    const {data:{Combination},QINIU_SERVER,beforeUpload,tokens,GroupState} = this.props
    ...
    const columns = [
      {
        ...
        render: (text,record,k) => {
          return (
            <div key={k}>
              <Input disabled={GroupState > 0 ? true : false} value={text} onChange={(e)=>{this.inputOnchange(e.target.value,k,'Name')}} style={{width:80}} placeholder="" />
            </div>
          )
        }
      },
      {
        ...
        render: (text,record,k) => {
          return (
            <div key={k}>
              <InputNumber disabled={GroupState > 0 ? true : false} value={text} onChange={(e)=>{this.inputOnchange(e,k,'MarketPrice')}} style={{width:50}} placeholder="" />
            </div>
          )
        }
      },
      {
        ...
        render: (text,record,k) => {
          return (
            <div key={k}>
              <InputNumber disabled={GroupState > 0 ? true : false} value={text} onChange={(e)=>{this.inputOnchange(e,k,'GroupBuyingPrice')}} style={{width:50}} placeholder="" />
            </div>
          )
        }
      },
      {
        ...
        render: (text,record,k) => {
          return (
            <div key={k}>
              <InputNumber disabled={GroupState > 0 ? true : false} value={text} onChange={(e)=>{this.inputOnchange(e,k,'SettlementPrice')}} style={{width:50}} placeholder="" />
      ...
      },
      {
        ...
        render: (text,record,k) => {
          return (
            <div key={k}>
              <InputNumber disabled={GroupState > 0 ? true : false} value={text} onChange={(e)=>{this.inputOnchange(e,k,'CostPrice')}} style={{width:50}} placeholder="" />
        ...
        render: (text,record,k) => {
          return (
            <div key={k} style={{width:80}}>
              <a disabled={GroupState > 0 ? true : false} onClick={()=>{this.openCombinationModel(k,text)}}>{text ? this.getSubSkuItems(text) : '关联单品'}</a>
            </div>
          )
        }
      },
      ...
      },
      {
        ...
        render: (text,record,k) => {
          return (
            <a disabled={GroupState > 0 ? true : false} onClick={()=>{this.setCombinationDelete(k)}}>删除</a>
          )
        }
      }
    ]
    return(
      <div>
        <div style={{padding:10,borderBottom:'1px solid #d9d9d9'}}>组合规格 <a style={{float:'right'}} disabled={GroupState > 0 ? true : false} onClick={()=>{this.setCombinationAdd()}}>添加组合</a></div>
