...
export default class Sku extends PureComponent {
  ...
  render(){
    ...
    const {data:{Specification,Sku},QINIU_SERVER,beforeUpload,tokens,form,GroupBuyingMode} = this.props
    const buttonStyle = {
      marginRight:10,
      marginLeft:10,
    ...
    const columns = [
      ...list1,{
        ...
        render: (text,record,k) => {
          return (
            <div key={k}>
              <InputNumber value={Sku[k].MarketPrice} style={{width:50}} onChange={(e)=>{this.inputOnchange(e,k,'MarketPrice')}}/>
            </div>
          )
        }
      },
      {
        ...
        render: (text,record,k) => {
          return (
            <div key={k}>
              <InputNumber value={Sku[k].GroupBuyingPrice} style={{width:50}} onChange={(e)=>{this.inputOnchange(e,k,'GroupBuyingPrice')}}/>
            </div>
          )
        }
      },
      {
        ...
        render: (text,record,k) => {
          return (
            <div key={k}>
              <InputNumber value={Sku[k].SettlementPrice} style={{width:50}} onChange={(e)=>{this.inputOnchange(e,k,'SettlementPrice')}}/>
            </div>
          )
        }
      },
      {
        ...
        render: (text,record,k) => {
          return (
            <div key={k}>
              <InputNumber value={Sku[k].CostPrice} style={{width:50}} onChange={(e)=>{this.inputOnchange(e,k,'CostPrice')}}/>
            </div>
          )
        }
      }
    ...
    if(GroupBuyingMode===2){
      columns.push({
        ...
        render: (text,record,k) => {
          return (
            <div key={k}>
              <InputNumber value={Sku[k].InventoryCount} style={{width:50}} onChange={(e)=>{this.inputOnchange(e,k,'InventoryCount')}}/>
            </div>
          )
        }
      })
    ...
    return(
      <div style={{border:'1px solid #d9d9d9'}}>
        <SpecificationView data={SpecificationData} dispatch={this.props.dispatch} getInstance={(childCp) => { this.childCp = childCp;}}/>
        <div style={{padding:10,borderBottom:'1px solid #d9d9d9'}}>单品规格 <a style={{float:'right'}} onClick={()=>{this.setSpecificationVisible()}}>修改或者添加</a></div>
        <div style={{padding:10}}>
          <div>批量填充:
            <InputNumber style={buttonStyle} onChange={(e)=>{this.fill(e,'MarketPrice')}} placeholder="市场价"/>
            <InputNumber style={buttonStyle} onChange={(e)=>{this.fill(e,'GroupBuyingPrice')}} placeholder="团购价"/>
            <InputNumber style={buttonStyle} onChange={(e)=>{this.fill(e,'SettlementPrice')}} placeholder="结算价"/>
            <InputNumber style={buttonStyle} onChange={(e)=>{this.fill(e,'CostPrice')}} placeholder="成本价"/>
          </div>
